...
func _ready() -> void:
	# emulate_mouse_from_touch=true allows menus to work on mobile; joystick still receives ScreenTouch
	if not ProjectSettings.get_setting("input_devices/pointing/emulate_touch_from_mouse"):
		printerr("The Project Setting 'emulate_touch_from_mouse' should be set to True (for desktop testing)")
	
	if not DisplayServer.is_touchscreen_available() and visibility_mode == Visibility_mode.TOUCHSCREEN_ONLY :
		hide()
	
	if visibility_mode == Visibility_mode.WHEN_TOUCHED:
		# Use modulate instead of hide() so the Control still receives input (hidden nodes don't)
		modulate.a = 0.0
...
func _move_base(new_position: Vector2) -> void:
	# Use Base's transform so pivot scales correctly when Base has custom scale/size
