...
@export var aura_ability_scene: PackedScene
...
func _get_ability_level() -> int:
	...
	return manager.get_ability_level("aura") if manager else 0

func _apply_stats_from_level() -> void:
	# TODO: map levels 1-9 to stats (debuffs via upgrades)
	pass
...
func _on_timer_timeout() -> void:
	if _get_ability_level() <= 0:
		return
	# TODO: spawn visual, apply damage (constant AOE around player)
	pass

func _on_ability_upgrade_added(upgrade: AbilityUpgrade, _current: Dictionary) -> void:
	if upgrade.ability_id == "aura":
		_apply_stats_from_level()
