...
@export var smite_ability_scene: PackedScene
...
func _get_ability_level() -> int:
	...
	return manager.get_ability_level("smite") if manager else 0
...
func _on_timer_timeout() -> void:
	if _get_ability_level() <= 0:
		return
	# TODO: non-standard trigger - enemies hit by other attacks; apply damage directly
	pass

func _on_ability_upgrade_added(upgrade: AbilityUpgrade, _current: Dictionary) -> void:
	if upgrade.ability_id == "smite":
		_apply_stats_from_level()
