...
@export var shadow_grab_ability_scene: PackedScene
...
func _get_ability_level() -> int:
	...
	return manager.get_ability_level("shadow_grab") if manager else 0
...
func _on_timer_timeout() -> void:
	if _get_ability_level() <= 0:
		return
	# TODO: highest health in range, multiple targets, stun/slow/DoT, re-attach when able
	pass

func _on_ability_upgrade_added(upgrade: AbilityUpgrade, _current: Dictionary) -> void:
	if upgrade.ability_id == "shadow_grab":
		_apply_stats_from_level()
