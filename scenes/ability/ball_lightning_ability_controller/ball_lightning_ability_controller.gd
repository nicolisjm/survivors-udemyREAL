...
func on_timer_timeout() -> void:
	...
	var normal_damage: float = (base_damage + flat_bonus) * damage_mult * overflow_bonus
	var crit_damage: float = (base_damage + flat_bonus + _crit_base_bonus) * 2.0 * damage_mult * overflow_bonus
	...
	ball.orbit_direction = 1.0 if randi() % 2 == 0 else -1.0
	ball.duration_mult = _duration_mult * duration_mult
	...
	ball.scale = Vector2.ONE * _size_mult * size_mult
	...
	if _spawn_forward_ball:
		...
		forward_ball.duration_mult = _duration_mult * duration_mult
		...
		forward_ball.scale = Vector2.ONE * _size_mult * size_mult
