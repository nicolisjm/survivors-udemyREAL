...
@export var tail_swipe_ability_scene: PackedScene
...
func _get_ability_level() -> int:
	...
	return manager.get_ability_level("tail_swipe") if manager else 0
...
func _on_timer_timeout() -> void:
	if _get_ability_level() <= 0:
		return
	# TODO: spawn visual (spinning rectangle around player, knockback)
	pass

func _on_ability_upgrade_added(upgrade: AbilityUpgrade, _current: Dictionary) -> void:
	if upgrade.ability_id == "tail_swipe":
		_apply_stats_from_level()
